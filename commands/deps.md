...
- `lodash` → Use specific imports or native methods
- `axios` → Consider native `fetch`
- `jquery` → Remove if only used for one thing
...
|---------|------------------|------------------|
| next | 14 → 15 | [Link to guide] |
| react | 18 → 19 | [Link to guide] |
...
echo "✅ Updates complete - verify app manually"
```
